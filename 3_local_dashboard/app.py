...
def index():
    ...
    categories_url = os.path.join(SITE_ROOT, 'static/data', 'categories.json')
    categories = json.load(open(categories_url))
    sorted_categories = sorted(categories.items(), key=operator.itemgetter(1), reverse=True)

    authors_url = os.path.join(SITE_ROOT, 'static/data', 'authors.json')
    authors = json.load(open(authors_url))
    sorted_authors = sorted(authors.items(), key=operator.itemgetter(1), reverse=True)

    weekdays_url = os.path.join(SITE_ROOT, 'static/data', 'weekdays.json')
    weekdays = json.load(open(weekdays_url))

    return render_template('index.html', stats=stats, categories=sorted_categories, authors=sorted_authors,
    weekdays=weekdays)
